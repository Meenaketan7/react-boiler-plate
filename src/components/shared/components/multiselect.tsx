...
interface MultiSelectProps
  ...
  animation?: number;
  maxCount?: number;
  maxSelectionLimit?: number; // Optional: Maximum number of items that can be selected
  modalPopover?: boolean;
  asChild?: boolean;
  className?: string;
  noResultText?: string;
  returnType?: 'string' | 'object';
  showLimitMessage?: boolean; // Optional: Whether to show limit message when limit is reached
}
  ...
  (
    {
      ...
      animation = 0,
      maxCount = 3,
      maxSelectionLimit, // No default - unlimited if not provided
      modalPopover = false,
      asChild = false,
      className,
    ...
    const hasSelectionLimit = maxSelectionLimit !== undefined;
    ...
    const toggleOption = (option: any) => {
      ...
      if (returnType === 'object') {
        ...
        if (exists) {
          ...
          // Selecting - check limit if it exists
          if (hasSelectionLimit && selectedValues.length >= maxSelectionLimit) {
    ...
    const toggleAll = () => {
      if (selectedValues.length === displayOptions.length) {
        ...
        // If there's a selection limit, only select up to that limit
        const optionsToSelect = hasSelectionLimit
          ? displayOptions.slice(0, maxSelectionLimit)
          : displayOptions;

        const allValues = optionsToSelect.map((option) =>
    ...
    // Check if option should be disabled (only when limit exists)
    const isOptionDisabled = (option: any) => {
      if (!hasSelectionLimit) return false;
    ...
    return (
      <div className="w-full">
          ...
          >
            <Command>
              <CommandList>
                ...
                <CommandGroup>
                  {/* Only show "Select All" if there's no selection limit */}
                  {!hasSelectionLimit && (
                    <CommandItem
                      key="all"
                      onSelect={toggleAll}
        ...
        {/* Show limit message only if limit exists and is reached */}
        {hasSelectionLimit &&
          showLimitMessage &&
