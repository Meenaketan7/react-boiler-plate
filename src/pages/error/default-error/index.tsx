...
export default function DefaultError({ error, reset }: DefaultErrorProps) {
	const message = typeof error === "string" ? error : (error && (error as any).message) || null;
			...
			<Card className="w-full max-w-3xl">
				...
				<CardContent>
					<div className="mt-2 text-sm text-muted-foreground">
						<p>If the problem persists, try reloading the page or contact support.</p>

						{debugText ? (
							<pre
								className="bg-surface mt-4 max-h-40 overflow-auto rounded-md p-3 text-left font-mono text-xs"
								aria-live="polite"
							>
								{debugText}
							</pre>
